...
run_case() { target=$1; mode=$2; output="$TEMP_ROOT/case.out"; rm -f "$LOG" "$SHELL_LOG" "$output"; : >"$CHECKOUT/probe.pyc"; : >"$ATTACKER_ROOT/keep.pyc"; set +e; case "$mode" in default) (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" WORKOUTPACT_COMMAND_LOG="$LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" "PYTHON=$FAKE_PYTHON" "XCODEBUILD=$FAKE_XCODEBUILD" "$target") >"$output" 2>&1;; command-root) (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" WORKOUTPACT_COMMAND_LOG="$LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" ROOT="$ATTACKER_ROOT" "PYTHON=$FAKE_PYTHON" "XCODEBUILD=$FAKE_XCODEBUILD" "$target") >"$output" 2>&1;; environment-root) (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" ROOT="$ATTACKER_ROOT" WORKOUTPACT_COMMAND_LOG="$LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" "PYTHON=$FAKE_PYTHON" "XCODEBUILD=$FAKE_XCODEBUILD" "$target") >"$output" 2>&1;; command-shell) (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" WORKOUTPACT_COMMAND_LOG="$LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" SHELL="$FAKE_SHELL" "PYTHON=$FAKE_PYTHON" "XCODEBUILD=$FAKE_XCODEBUILD" "$target") >"$output" 2>&1;; environment-shell) (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" SHELL="$FAKE_SHELL" WORKOUTPACT_COMMAND_LOG="$LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" "PYTHON=$FAKE_PYTHON" "XCODEBUILD=$FAKE_XCODEBUILD" "$target") >"$output" 2>&1;; esac; status=$?; set -e; if [ "$status" -ne 0 ] || [ -e "$SHELL_LOG" ] || [ ! -e "$ATTACKER_ROOT/keep.pyc" ]; then printf 'authority case failed: target=%s mode=%s status=%s\n' "$target" "$mode" "$status" >&2; cat "$output" >&2; return 1; fi; case "$target" in clean) [ ! -e "$CHECKOUT/probe.pyc" ];; *) grep -Fq "$CHECKOUT" "$LOG";; esac; }
...
build check clean lint root-test test verify:
	@/usr/bin/touch "$$WORKOUTPACT_LATER_MARKER"
LATER_MAKE
for target in build check clean lint root-test test verify; do
  rm -f "$LATER_MARKER"
  if (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" WORKOUTPACT_LATER_MARKER="$LATER_MARKER" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER" "$target" "PYTHON=$FAKE_PYTHON" XCODEBUILD=/definitely/not-xcodebuild) >"$TEMP_ROOT/later-$target" 2>&1; then exit 1; fi
...
if ! (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" WORKOUTPACT_COMMAND_LOG="$LOG" WORKOUTPACT_LATER_ROOT_MARKER="$LATER_ROOT_MARKER" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER_VARS" root-test "PYTHON=$FAKE_PYTHON" XCODEBUILD=/definitely/not-xcodebuild) >"$TEMP_ROOT/later-root" 2>&1; then cat "$TEMP_ROOT/later-root" >&2; exit 1; fi
grep -Fq "$CHECKOUT" "$LOG"
[ ! -e "$LATER_ROOT_MARKER" ]
rm -f "$LOG"
if ! (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" WORKOUTPACT_COMMAND_LOG="$LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER_VARS" lint "PYTHON=$FAKE_PYTHON" XCODEBUILD=/definitely/not-xcodebuild) >"$TEMP_ROOT/later-python" 2>&1; then cat "$TEMP_ROOT/later-python" >&2; exit 1; fi
grep -Fq "$FAKE_PYTHON" "$LOG"
...
if ! (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" WORKOUTPACT_COMMAND_LOG="$LOG" WORKOUTPACT_LATER_SHELL_LOG="$LATER_SHELL_LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER_SHELL" check "PYTHON=$FAKE_PYTHON" XCODEBUILD=/definitely/not-xcodebuild) >"$TEMP_ROOT/later-shell" 2>&1; then cat "$TEMP_ROOT/later-shell" >&2; exit 1; fi
grep -Fq "$CHECKOUT" "$LOG"
[ ! -e "$LATER_SHELL_LOG" ]
...
(cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" WORKOUTPACT_LATER_SHELL_LOG="$LATER_SHELL_LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER_OVERRIDE" check "PYTHON=$FAKE_PYTHON" XCODEBUILD=/definitely/not-xcodebuild) >"$TEMP_ROOT/later-override" 2>&1
[ -s "$LATER_SHELL_LOG" ]
PATH_PYTHON="$TEMP_ROOT/python3"; PATH_PYTHON_LOG="$TEMP_ROOT/path-python.log"; cp "$FAKE_PYTHON" "$PATH_PYTHON"
rm -f "$PATH_PYTHON_LOG"
(cd "$CONTROL_DIR"&&PATH="$TEMP_ROOT:/usr/bin:/bin" WORKOUTPACT_COMMAND_LOG="$PATH_PYTHON_LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" lint) >"$TEMP_ROOT/path-python" 2>&1
[ -s "$PATH_PYTHON_LOG" ]
PATH_XCODE="$TEMP_ROOT/xcodebuild"; PATH_XCODE_LOG="$TEMP_ROOT/path-xcode.log"; cat >"$PATH_XCODE" <<'SCRIPT'
#!/bin/sh
printf '%s\n' "$*" >> "$WORKOUTPACT_PATH_XCODE_LOG"
exit 0
SCRIPT
chmod +x "$PATH_XCODE"
rm -f "$PATH_XCODE_LOG"
(cd "$CONTROL_DIR"&&PATH="$TEMP_ROOT:/usr/bin:/bin" WORKOUTPACT_PATH_XCODE_LOG="$PATH_XCODE_LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" build "PYTHON=$FAKE_PYTHON" XCODEBUILD=/definitely/not-xcodebuild) >"$TEMP_ROOT/path-xcode" 2>&1
[ ! -e "$PATH_XCODE_LOG" ]
EXPLICIT_XCODE="$TEMP_ROOT/explicit xcodebuild"; EXPLICIT_XCODE_LOG="$TEMP_ROOT/explicit-xcode.log"; cat >"$EXPLICIT_XCODE" <<'SCRIPT'
#!/bin/sh
printf '%s\n' "$*" >> "$WORKOUTPACT_EXPLICIT_XCODE_LOG"
exit 0
SCRIPT
chmod +x "$EXPLICIT_XCODE"
rm -f "$EXPLICIT_XCODE_LOG" "$TARGET_XCODE_LOG"
(cd "$CONTROL_DIR"&&PATH="$TEMP_ROOT:/usr/bin:/bin" XCODEBUILD="$EXPLICIT_XCODE" WORKOUTPACT_EXPLICIT_XCODE_LOG="$EXPLICIT_XCODE_LOG" WORKOUTPACT_TARGET_XCODE_LOG="$TARGET_XCODE_LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER_VARS" build "PYTHON=$FAKE_PYTHON") >"$TEMP_ROOT/explicit-xcode" 2>&1
